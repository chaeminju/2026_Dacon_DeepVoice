...
def predict_presence_for_all_files(separated_audio, device):
    """HTDemucs로 분리된 voice_audio/music_audio 각각을 PANNs에 독립적으로 입력해
    voice_present_prob, music_present_prob를 계산한다."""
    model, voice_indices, music_indices = load_panns_model(device)
    presence_scores = {}

    start_time = time.perf_counter()
    for audio_id, (voice_audio, music_audio) in tqdm(
        separated_audio.items(), total=len(separated_audio), desc="Presence (PANNs)"
    ):
        voice_present = predict_presence(model, voice_indices, voice_audio)
        music_present = predict_presence(model, music_indices, music_audio)
        presence_scores[audio_id] = (voice_present, music_present)
    elapsed = time.perf_counter() - start_time
    num_files = max(len(separated_audio), 1)
    print(
        f"[Timing] PANNs presence (separated input): {elapsed:.2f}s total, "
        f"{elapsed / num_files:.3f}s/file"
    )
...
def separate_all_files(audio_files, device):
    """HTDemucs 분리를 파일당 한 번만 실행하고, 그 결과(voice_audio, music_audio)를
    PANNs(존재 판별)와 DF-Arena(fake 판별) 양쪽에서 재사용할 수 있도록 반환한다."""
    htdemucs_model = load_htdemucs_model()
    separated_audio = {}

    start_time = time.perf_counter()
    for audio_path in tqdm(audio_files, desc="Separation (HTDemucs)"):
        separated_audio[audio_path.stem] = separate_voice_and_music(
            audio_path, htdemucs_model, device
        )
    elapsed = time.perf_counter() - start_time
    num_files = max(len(audio_files), 1)
    print(
        f"[Timing] HTDemucs separation: {elapsed:.2f}s total, "
        f"{elapsed / num_files:.3f}s/file"
    )
...
def load_df_arena_model(device):
    ...
    # from_pretrained가 일부 커스텀 파라미터(backbone.conformer.positional_emb)를
    # 체크포인트 dtype과 다르게 fp32로 남겨두는 경우가 있어(파인튜닝 후 fp16으로
    # 저장한 체크포인트 로딩 시 재현됨), 전체 파라미터/버퍼를 대표 dtype으로 강제 통일한다.
    target_dtype = next(model.backbone.ssl_model.parameters()).dtype
...
def predict_fake_scores_for_all_files(
    audio_files, submission_rows, presence_scores, separated_audio, device
):
    """DF-Arena 1B의 입력과 로직은 기존과 동일하게, HTDemucs로 이미 분리된
    voice_audio/music_audio를 그대로 재사용한다 (HTDemucs 재실행 없음)."""
    df_arena_model, fake_label_index = load_df_arena_model(device)

    start_time = time.perf_counter()
    for index, audio_path in enumerate(tqdm(audio_files, desc="Components (DF-Arena)")):
        voice_audio, music_audio = separated_audio[audio_path.stem]
    ...
    num_files = max(len(audio_files), 1)
    print(
        f"[Timing] DF-Arena fake scoring: {elapsed:.2f}s total, "
        f"{elapsed / num_files:.3f}s/file"
    )
...
def main():
    ...
    # 2. HTDemucs로 음성·음악을 먼저 분리한다 (한 번만 실행).
    separated_audio = separate_all_files(audio_files, device)

    # 3. 분리된 voice_audio/music_audio 각각을 PANNs에 입력해 존재 확률을 계산한다.
    presence_scores = predict_presence_for_all_files(separated_audio, device)

    # 4. 분리된 voice_audio/music_audio 각각을 DF-Arena 1B에 입력해
    #    성분별 Fake 확률을 계산한다 (HTDemucs 결과 재사용, 재실행 없음).
    ...
    print(
        f"[Timing] Total pipeline: {total_elapsed:.2f}s for {len(audio_files)} files "
        f"({total_elapsed / max(len(audio_files), 1):.3f}s/file)"
    )
