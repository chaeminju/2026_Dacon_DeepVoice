...
# [벤더링 수정] 이 오프라인 패키지는 SpecTTTra 체크포인트만 쓰고 ViT/timm
# 인코더 분기(get_encoder() 아래 참고)는 타지 않는다. ViT는 timm.layers의
# PatchEmbed를, "timm" 분기는 timm 자체를 필요로 하는데, timm은 torchvision을
# 끌고 와 채점 서버의 torch/torchaudio 빌드를 깨뜨릴 수 있어(_timm_compat.py
# 주석 참고) 최상단 import 대신 실제로 그 분기를 탈 때만 지연 import한다.
