...
# [벤더링 수정] 원본은 CutMix(미사용 클래스)를 위해 torchvision을 모듈
# 최상단에서 import한다. 이 추론 전용 배포본에서는 AugmentLayer가 MixUp/
# SpecAugment만 사용하고(추론 시 self.training=False라 애초에 호출되지도
# 않음) CutMix는 어디서도 인스턴스화되지 않으므로, 불필요한 torchvision
# 의존성을 없애기 위해 CutMix 내부로 지연 import한다.
...
class CutMix(torch.nn.Module):
    ...
    def forward(self, batch: Tensor, target: Tensor) -> Tuple[Tensor, Tensor]:
        ...
        from torchvision.transforms import functional as F  # 지연 import (위 주석 참고)
