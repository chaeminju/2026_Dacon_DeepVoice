...
원본 오디오
  ...
  → voice_audio → DF-Arena 1B(zero-shot) → voice_fake_df
  → voice_audio → AntiDeepfake MMS-1B(zero-shot) → voice_fake_mms
  → VOICE_FAKE_PROB = (voice_fake_df + voice_fake_mms) / 2  (단순 평균 앙상블)
  → music_audio → SpecTTTra → MUSIC_FAKE_PROB
  → combine_file_fake_score()로 FILE_FAKE_PROB 계산

실험 목적: voice 판별을 DF-Arena 단독에서 "DF-Arena + AntiDeepfake MMS-1B 평균"
으로 바꿔서, 단독 교체(DF-Arena 1B 0.7186 > XLS-R-2B 0.7062 > MMS-1B 0.6962)가
전부 졌던 것과 달리 평균(ensemble_aasist3 방식)으로는 개선되는지 확인한다.
음악/presence/분리는 기존과 동일하게 유지해 voice 앙상블 효과만 분리해서 본다.

HTDemucs·PANNs·DF-Arena 1B·AntiDeepfake MMS-1B·SpecTTTra 다섯 모델은 전체 파일
루프 시작 전 딱 한 번만 로딩하고, 파일별로 한 바퀴만 도는 단일 루프 안에서 재사용한다.
"""
...
DF_ARENA_DIR = MODEL_DIR / "df_arena_1b"  # DF-Arena 1B (zero-shot)
ANTIDEEPFAKE_MMS1B_DIR = MODEL_DIR / "antideepfake_mms1b"  # AntiDeepfake MMS-1B (zero-shot), voice 앙상블용 두 번째 모델
...
def parse_arguments():
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "파라미터 수가 큰 DF-Arena 1B와 AntiDeepfake MMS-1B를 평소 CPU에 "
            "두고, 실제로 호출하는 순간에만 device로 옮겼다가 즉시 CPU로 "
            "되돌린다(PANNs/HTDemucs/SpecTTTra는 용량이 작아 항상 device에 "
            "상주). GPU 메모리가 부족한 환경(예: GPU RAM 8GB 이하)에서 "
            "사용한다. 기본값은 다섯 모델 모두 device에 동시 상주(더 빠름)."
        ),
    )
...
# -----------------------------------------------------------------------------
# 5. voice 판별 두 모델(DF-Arena 1B, AntiDeepfake MMS-1B) 로딩 및 Fake 추론
# -----------------------------------------------------------------------------
# 두 모델 모두 predict_fake()로 voice_audio 하나를 각각 독립적으로 채점하고,
# main()에서 두 점수를 평균해 VOICE_FAKE_PROB로 쓴다(단순 확률 평균 앙상블).

def load_df_arena_model(device):
    """baseline_submit과 동일한 zero-shot DF-Arena 1B 로더(변경 없음)."""
...
def load_antideepfake_mms1b_model(device):
    """voice 앙상블의 두 번째 모델: AntiDeepfake MMS-1B(fp32, zero-shot)."""
    if str(ANTIDEEPFAKE_MMS1B_DIR) not in sys.path:
        sys.path.insert(0, str(ANTIDEEPFAKE_MMS1B_DIR))
    from detector import AntiDeepfakeDetector, FAKE_LABEL_INDEX

    model = AntiDeepfakeDetector(ANTIDEEPFAKE_MMS1B_DIR).to(device=device).eval()
    return model, FAKE_LABEL_INDEX
...
# -----------------------------------------------------------------------------
# 다섯 모델의 GPU 메모리 사용량을 합치면(PANNs Cnn14 ~300MB, HTDemucs ~80MB,
# SpecTTTra 17M 파라미터 ~70MB, DF-Arena 1B fp32 ~4.3GB, AntiDeepfake MMS-1B
# fp32 ~3.9GB) 대략 9GB 안팎이라, L4(22.4GiB) 등 GPU RAM이 넉넉한 환경이라면
# 다섯 모델을 동시에 device에 상주시켜도 60분/1,200개 처리에 무리가 없다.
# 파라미터 수 기준으로는 DF-Arena 1B와 MMS-1B가 지배적이므로, --low-memory
# 옵션을 켜면 이 둘만 평소 CPU에 두고 실제로 호출하는 순간에만 device로
# 옮겼다가 바로 CPU로 되돌린다(가장 작은 PANNs/SpecTTTra는 굳이 옮길 필요가
# 없어 항상 device에 상주). HTDemucs는 원래 apply_model() 호출 시점에만
# device를 사용하는 구조라 그대로 둔다.
...
def main():
    ...
    # 2. 다섯 모델(HTDemucs, PANNs, DF-Arena 1B, AntiDeepfake MMS-1B, SpecTTTra)을
    #    루프 시작 전 딱 한 번만 로딩한다. 루프 안에서는 재사용만 하고 반복 로딩하지 않는다.
    print("Loading models: HTDemucs, PANNs, DF-Arena 1B, AntiDeepfake MMS-1B, SpecTTTra ...")
    htdemucs_model = load_htdemucs_model()
    # PANNs·SpecTTTra는 용량이 작아 --low-memory에서도 device에 항상 상주시킨다
    # (DF-Arena 1B/MMS-1B만 토글 대상, 위 8번 섹션 설명 참고).
    panns_model, voice_indices, music_indices = load_panns_model(device)
    df_arena_model, df_arena_fake_index = load_df_arena_model(
        cpu_device if args.low_memory else device
    )
    mms1b_model, mms1b_fake_index = load_antideepfake_mms1b_model(
        cpu_device if args.low_memory else device
    )
    ...
    timings = {
        ...
        "AntiDeepfake MMS-1B (voice fake)": 0.0,
    ...
    for index, audio_path in enumerate(
        ...
        # 3-3. voice_audio → DF-Arena 1B, AntiDeepfake MMS-1B 각각 채점 후 평균
        #      → VOICE_FAKE_PROB (단순 확률 평균 앙상블)
        if args.low_memory:
            move_model(df_arena_model, device)
        t0 = time.perf_counter()
        voice_fake_df = predict_fake(
            df_arena_model, df_arena_fake_index, voice_audio, device
        )
        ...
        if args.low_memory:
            move_model(mms1b_model, device)
        t0 = time.perf_counter()
        voice_fake_mms1b = predict_fake(
            mms1b_model, mms1b_fake_index, voice_audio, device
        )
        timings["AntiDeepfake MMS-1B (voice fake)"] += time.perf_counter() - t0
        if args.low_memory:
            move_model(mms1b_model, cpu_device)

        voice_fake = (voice_fake_df + voice_fake_mms1b) / 2.0
    ...
    del htdemucs_model, panns_model, df_arena_model, mms1b_model, spectttra_model
    if device.type == "cuda":
